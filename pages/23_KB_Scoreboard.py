...
st.markdown("""
    <style>
        ...
        /* 💡 प्रोफेसनल पप-अप म्यासेज विथ आइकन (TV Size) */
        .court-popup {
...
# ==========================================
# 📡 Fetch Live Kabaddi Data
# ==========================================
try:
    conn = db.get_connection()
    m_row = conn.execute("SELECT score_summary FROM matches WHERE score_summary IS NOT NULL AND status != 'Completed' ORDER BY match_id DESC LIMIT 1").fetchone()
    
    if m_row and m_row['score_summary']:
        state = json.loads(m_row['score_summary'])
    ...
    state = None; st.error(f"Error loading data: {e}")
finally:
    try: conn.close()
    except: pass
...
if not state or not state.get('match_started'):
    st.markdown("<div style='text-align:center; margin-top:15vh;'><h1 style='font-size:100px; color:#cbd5e1;'>🤼 KABADDI LIVE</h1><p style='font-size:40px; color:#64748b;'>Waiting for match to start...</p></div>", unsafe_allow_html=True)
    st.stop()

# ==========================================
# 🔄 💡 १. TV Mirroring Logic (Audience Perspective)
# ==========================================
c_half = state.get('half', 1)
op_swap = state.get('swap_sides', False)

# Operator's View
op_left = p2 if op_swap else p1
op_right = p1 if op_swap else p2
...
l_role = "🏃 रेडर (Raider)" if active_r_team == left_team else "🛡️ डिफेन्स (Defence)"
l_role_bg = "#2563eb" if active_r_team == left_team else "#475569"
r_role = "🏃 रेडर (Raider)" if active_r_team == right_team else "🛡️ डिफेन्स (Defence)"
...
with c_score_L:
    st.markdown(f"""
        <div style='background:#1e293b; border-bottom:12px solid #2563eb; padding:20px; border-radius:20px; text-align:center; box-shadow: 0 10px 20px rgba(0,0,0,0.6);'>
            <h2 style='color:#93c5fd; margin:0; font-size:45px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;'>{left_team}</h2>
            <div style='font-size:120px; font-weight:900; color:white; line-height:1; text-shadow: 4px 4px 10px rgba(0,0,0,0.8); margin: 10px 0;'>{score_left}</div>
            <div style='background:{l_role_bg}; color:white; font-size:20px; font-weight:bold; padding:5px 20px; border-radius:20px; display:inline-block;'>{l_role}</div>
...
with c_timer:
    ...
    st.components.v1.html(f"""
        <div style="text-align:center; font-family:monospace;">
            <div style="background:#0f172a; border:3px solid #475569; color:#cbd5e1; padding:8px; border-radius:15px; display:inline-block; font-size:28px; font-weight:bold; margin-bottom:15px;">HALF {c_half}</div>
        ...
        <script>
            let s = parseInt(sessionStorage.getItem('m_sec')) || 1200; 
            ...
            let r_run = {raid_active};

            function update() {{ 
                if(running && s>0) {{ s--; sessionStorage.setItem('m_sec', s); }}
...
with c_score_R:
    st.markdown(f"""
        <div style='background:#1e293b; border-bottom:12px solid #dc2626; padding:20px; border-radius:20px; text-align:center; box-shadow: 0 10px 20px rgba(0,0,0,0.6);'>
            <h2 style='color:#fca5a5; margin:0; font-size:45px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;'>{right_team}</h2>
            <div style='font-size:120px; font-weight:900; color:white; line-height:1; text-shadow: 4px 4px 10px rgba(0,0,0,0.8); margin: 10px 0;'>{score_right}</div>
            <div style='background:{r_role_bg}; color:white; font-size:20px; font-weight:bold; padding:5px 20px; border-radius:20px; display:inline-block;'>{r_role}</div>
...
# ==========================================
# 🤼 KABADDI COURT RENDER LOGIC (TV VERSION)
# ==========================================
# 💡 Because we mirrored left_team and right_team, lineup data mapped to them automatically flips visually!
...
# 💡 Attack Direction Logic for TV
raider_team = state.get('raider_team')
tv_is_attacking_right = (raider_team == left_team) # If TV's left team raids, they go right.
...
# 💡 २. Icon Flipping & Overlay Injection
if state.get('last_event_msg'):
    ...
    if state.get('last_event_icon'):
        ...
        if b64_str:
            icon_cls = "popup-icon"
            # 💡 Flip directional icons if action relates to the team on the Right side of the TV
    ...
    # 🔊 Sound Playback Logic (Triggers automatically via JS)
    st.components.v1.html("""
        <script>
            // Note: Browsers may block audio if TV screen hasn't been clicked at least once!
    ...
    # We do NOT reset state here on the TV! The operator panel resets it. 
    # Otherwise, rapid refreshes might cause it to disappear before TV clients see it.

html += f'<div class="mid-line"></div>'
html += f'<div class="baulk-line-left" style="background-color:{c_baulk_l}; box-shadow:0 0 10px {c_baulk_l};"></div>'
html += f'<div class="bonus-line-left" style="background-color:{c_bonus_l}; box-shadow:0 0 10px {c_bonus_l};"></div>'
html += f'<div class="baulk-line-right" style="background-color:{c_baulk_r}; box-shadow:0 0 10px {c_baulk_r};"></div>'
html += f'<div class="bonus-line-right" style="background-color:{c_bonus_r}; box-shadow:0 0 10px {c_bonus_r};"></div>'
